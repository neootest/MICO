...
__weak void EasyLinkWillStart( mico_Context_t * const inContext )
{
  return;
}
__weak void EasyLinkWillStop(mico_Context_t * const inContext )
{
  return; 
}

__weak OSStatus EasyLinkRecvAuthData(char * userInfo, mico_Context_t * const inContext )
{
  return true; 
}
...
void EasyLinkNotify_EasyLinkGetExtraDataHandler(int datalen, char* data, mico_Context_t * const inContext)
{
  ...
  require_noerr(EasyLinkRecvAuthData(data, inContext), exit);
...
OSStatus startEasyLink( mico_Context_t * const inContext)
{
  ...
  err = mico_add_notification( mico_notify_WIFI_STATUS_CHANGED, (void *)EasyLinkNotify_WifiStatusHandler );
  require_noerr(err, exit);
  err = mico_add_notification( mico_notify_WiFI_PARA_CHANGED, (void *)EasyLinkNotify_WiFIParaChangedHandler );
  require_noerr(err, exit);
  err = mico_add_notification( mico_notify_EASYLINK_COMPLETED, (void *)EasyLinkNotify_EasyLinkCompleteHandler );
  require_noerr(err, exit);
  err = mico_add_notification( mico_notify_EASYLINK_GET_EXTRA_DATA, (void *)EasyLinkNotify_EasyLinkGetExtraDataHandler );
  require_noerr(err, exit);
  err = mico_add_notification( mico_notify_DHCP_COMPLETED, (void *)EasyLinkNotify_DHCPCompleteHandler );
  require_noerr( err, exit );    
  err = mico_add_notification( mico_notify_SYS_WILL_POWER_OFF, (void *)EasyLinkNotify_SYSWillPoerOffHandler );
  require_noerr( err, exit ); 
...
OSStatus _connectFTCServer( mico_Context_t * const inContext, int *fd)
{
  ...
  *fd = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
  ...
  err = EasyLinkCreateReportJsonHTTPMessage( inContext );
  require_noerr( err, exit );
...
void _cleanEasyLinkResource( mico_Context_t * const inContext )
{
  ...
  mico_remove_notification( mico_notify_WIFI_STATUS_CHANGED, (void *)EasyLinkNotify_WifiStatusHandler );
  mico_remove_notification( mico_notify_WiFI_PARA_CHANGED, (void *)EasyLinkNotify_WiFIParaChangedHandler );
  mico_remove_notification( mico_notify_EASYLINK_COMPLETED, (void *)EasyLinkNotify_EasyLinkCompleteHandler );
  mico_remove_notification( mico_notify_EASYLINK_GET_EXTRA_DATA, (void *)EasyLinkNotify_EasyLinkGetExtraDataHandler );
...
void easylink_thread(void *inContext)
{
  ...
  while(1){
    if(Context->micoStatus.easylinkClient_fd == -1){
      ...
      FD_SET(Context->micoStatus.easylinkClient_fd, &sendfds);

      err = select(1, &readfds, &sendfds, NULL, &t);
...
/*Module is ignoreed by FTC server, */    
threadexit:
  EasyLinkWillStop( Context );
    ...
    switch(inHeader->statusCode){
      ...
      case kStatusOK:
        ...
        if( strnicmpx( value, valueSize, kMIMEType_JSON ) == 0 ){
          ...
          err = EasyLinkIncommingJsonHTTPMessage( inHeader->extraDataPtr, inContext);
          close(fd);
...
OSStatus FTC_AddTopMenu(json_object **outTopMenu, char* const inName, json_object* inlowerSectorArray, OTA_Versions_t inVersions)
{
  OSStatus err;
  json_object *object;
  err = kNoErr;
