...
OSStatus EasyLinkCreateReportJsonHTTPMessage( mico_Context_t * const inContext )
{
  ...
  require_noerr(err, exit);
    ...
    require_noerr(err, exit);
      ...
      require_noerr(err, exit);
        ...
        err = FTC_AddStringCellToSector(subMenuSector, "RF Driver Rev.", rfVer,                             "RO", NULL);
        require_noerr(err, exit);
  ...
  sector = json_object_new_array();
  require( sector, exit );
  err = FTC_AddSector(sectors, "SPP Remote Server",           sector);
  require_noerr(err, exit);


    // SPP protocol remote server connection enable
    err = FTC_AddSwitchCellToSector(sector, "Connect SPP Server",   inContext->flashContentInRam.appConfig.remoteServerEnable,   "RW");
    require_noerr(err, exit);

    //Seerver address cell
    err = FTC_AddStringCellToSector(sector, "SPP Server",           inContext->flashContentInRam.appConfig.remoteServerDomain,   "RW", NULL);
    require_noerr(err, exit);

    //Seerver port cell
    err = FTC_AddNumberCellToSector(sector, "SPP Server Port",      inContext->flashContentInRam.appConfig.remoteServerPort,   "RW", NULL);
    require_noerr(err, exit);
