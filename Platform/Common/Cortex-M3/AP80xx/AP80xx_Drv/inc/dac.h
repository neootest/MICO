 ...
 * @return  None
 * @note	1).??ClassD???,??DacAdcSampleRateSet(SampRate, NORMAL_MODE)?
 *			   ClkMclkSelBySampleRate(SampRate, NORMAL_MODE)????????MCLK?????
 *			2).??CLASSD???8000/11025/12000??????,??????DAC???????
 *			   ???????ADC?????????IIS???ClassD???????????PHUB
 *			   ????OFIFOIN_MIX_DACOUT??,????????I2S_CTL0???(I2S_SEL??1)
 *			   ??ADCIN to IISOUT????????DAC?ADC???????,IIS??????
 *			   ???ADC????,8000/12000??????ADC/IIS?????????48KHz,
 *			   11025????ADC/IIS?????????44100Hz?
 */
 ...
 * @param	1: usb mode(12M), 0: normal mode
 * @return	none
 */
