/*
 * @brief LPC412X ADC ROM API declarations and functions
 *
 * @note
...
#ifndef __ROM_ADC_412X_H_
#define __ROM_ADC_412X_H_

#ifdef __cplusplus
extern "C" {
#endif
	
/** @defgroup ADCD_412X CHIP: LPC412X ADC ROM API declarations and functions
 * @ingroup CHIP_412X_Drivers
 * @{
 */

/**
 * @brief LPC412X ADC ROM APIs - set_pll mode options
 */
typedef void*  ADC_HANDLE_T; // define TYPE for spi handle pointer
...
typedef struct   {   // block of RAM allocated by the application program
	LPC_ADC_T*        base_addr    ;  // adcr register base address
...
} ADC_DRIVER_TypeDef;  //HEADER_TypeDef	 *********************************
...
typedef ErrorCode_t  (*ADC_DMA_SETUP_T) (ADC_HANDLE_T handle, ADC_DMA_CFG_T *dma_cfg); //define SPI DMA channel setup func TYPE
...
//     SPI functions.  One structure is used to pass params to most of the functions.  
...
extern const ADCD_API_T  adc_api ; //so application program can access	pointer to
                                     // function table
