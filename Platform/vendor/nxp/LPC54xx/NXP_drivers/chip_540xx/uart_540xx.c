...
static CHIP_ASYNC_SYSCTL_CLOCK_T getUARTClockID(LPC_USART_T *pUART)
{
	if (pUART == LPC_USART0) {
		return ASYNC_SYSCTL_CLOCK_UART0;
	}
	else if (pUART == LPC_USART1) {
		return ASYNC_SYSCTL_CLOCK_UART1;
	}
	else if (pUART == LPC_USART2) {
		return ASYNC_SYSCTL_CLOCK_UART2;
	}
	return ASYNC_SYSCTL_CLOCK_UART3;
}
...
{
	/* Enable USART clock */
	Chip_Clock_EnableAsyncPeriphClock(getUARTClockID(pUART));

	/* UART reset */
	if (pUART == LPC_USART0) {
		/* Peripheral reset control to USART0 */
		Chip_SYSCTL_AsyncPeriphReset(ASYNC_RESET_UART0);
	}
	else if (pUART == LPC_USART1) {
		/* Peripheral reset control to USART1 */
		Chip_SYSCTL_AsyncPeriphReset(ASYNC_RESET_UART1);
	}
	else if (pUART == LPC_USART2) {
		/* Peripheral reset control to USART2 */
		Chip_SYSCTL_AsyncPeriphReset(ASYNC_RESET_UART2);
	} 
	else if (pUART == LPC_USART3) {
		/* Peripheral reset control to USART3 */
		Chip_SYSCTL_AsyncPeriphReset(ASYNC_RESET_UART3);
	} 
...
{
	/* Enable USART clock */
	Chip_Clock_DisableAsyncPeriphClock(getUARTClockID(pUART));
}
